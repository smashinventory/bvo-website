...
## Colour Families — Ash reclassified, word boundaries, dual buckets, and a live outage
**Date:** 2026-09-16
**Commits:** `3a302ab`, `19161f0`, hotfix for the collation failure
**Migration:** `migrations/2026-09-16_mirror_colour_backfill.sql` (generated by `scripts/gen_mirror_colour_sql.js`)
**Fixture:** `test/fixtures/jm_feed_colors_2026-07-15.json` — the 96 feed colour strings, frozen for the regression gate

### The original problem
112 of 120 active mirrors had `color_family = NULL`, so the mirrors collection
page offered a colour filter that matched almost nothing.

**Root cause, one line.** `importJamesMartinFeed.js` read colour from a single
feed column:

    const rawColor = clean(row['Vanity Base Color/Finish']);

JM puts the finish in a different column depending on product type —
`Vanity Base Color/Finish` (col 134) for vanities and cabinets,
`Finish/Color of Product` (col 135) for mirrors, faucets and accessories.
Verified against the feed: **all 120 mirrors populate col 135; exactly 8 also
populate col 134** — and those 8 were precisely the 8 that had a colour.
Nothing downstream was broken. The value never arrived.

Col 135 was documented as a JM colour column in `BVO_ISSUE1_BRIEF.md` line 58
long before this. It was not missing; it was walked past.

### Three fixes in the taxonomy

**1. `Ash` moved from `gray` to `wood_l`.** Ash is a pale hardwood, not the
residue of a fire. The tell was already in the file: `wood_l` carried
`Ash Wood`, `White Ash`, `Natural White Ash`, `Whitewashed Ash` and
`Natural Ash` as five separately hand-added members, each one a workaround for
the root classification being wrong. `Ash Gray` still resolves to `gray` —
`Gray` is 4 characters to `Ash`'s 3 and the longest member wins.

**2. `normalize()`'s substring fallback now requires a word boundary.** It used
`String.includes()`, which matches inside words:

    'Sunwashed Oak'    contains 'ash'   -> Sunw(ash)ed  -> gray
    'Oyster Shagreen'  contains 'green' -> Sha(green)   -> green

57 wood products were filed under Gray and Green by those two matches. Neither
member is wrong; a match beginning mid-word is simply not a match. Hyphens and
slashes still count as boundaries, so `off-white` matches `white`.

**Third occurrence of this exact mistake on this project** — a `LIKE '%LED%'`
that counted `contro(lled)` and `insta(lled)` as lighted mirrors, and a brand
comparison on a fragment. Assume word boundaries in any new membership test.

**3. A colour may now fill more than one bucket.** Sam, 2026-09-16: *"a person
looking for cream may be presented with a few brass options and like one. I
just want a decent balance of options that are very close to what the shopper
wants."*

Ten colour strings, 65 products, 1% of the catalogue. Vanities are untouched
apart from Matte Black and the Celeste run.

`DUAL_BUCKET` in `colorFamilies.js` is an explicit list, not a rule. The
obvious rule — *when the cabinet and metal contexts disagree, use both* — was
written, measured against the feed, and **rejected**: it gets `Silver Oak`
wrong, where the contexts disagree but the product is a wood and `chrome`
would be nonsense. `Silver Oak` is carried with `alt: []` so a later reader can
see it was considered and declined rather than missed.

### Storage — two questions, two homes
`products.color_family` holds ONE primary: the card swatch, and the canonical
answer to *what colour is this* (Rule 10). Additional swatches are EAV
`color_family_alt` rows — the same multi-value shape `insertStyleAttrs()`
already uses for `style`. That is two different facts, not one fact in two
places.

`resolveBuckets(raw, context, adminMappings)` is now the single entry point.
Priority: `color_mappings` (admin, via the Color Report) → `DUAL_BUCKET` →
`normalize()`. Importers call this, never `normalize()` directly (Rule 8).

### ⛔ THE OUTAGE — read this before writing any cross-table query

`19161f0` made swatch visibility a `UNION` between `products.color_family` and
`product_attribute_values.value_text`, so a swatch would appear when its only
products reach it as an alt. **Those two columns have different collations.**

    Error: Illegal mix of collations for operation 'UNION'
    at collectionsController.js:881

`exports.show` threw for **every** `/collections/:slug`, not only mirrors. The
site was down until the hotfix deployed.

Fixed by splitting it into two queries merged with a `Set` in JS — no
cross-column comparison, so the collation cannot matter. `COLLATE` would also
work and was rejected: it hard-codes into application code a collation the
application has no way to know.

**Nine gates passed on that commit and not one could have caught it.** They
parse JavaScript, match strings and resolve colours in-process. None executes
SQL against MySQL. A query that is valid JS, plausible SQL and rejected by the
server was invisible to all of them. See Rule 14 in `BVO_AUDIT_BRIEF.md`.

### Applied by SQL, not by re-import
The importer fix only takes effect when the importer runs, and a full run
rewrites 5,218 products — price, MAP, inventory, images, bullets,
certifications — to populate two columns on 111 mirrors. Sam declined that
blast radius, correctly.

`scripts/gen_mirror_colour_sql.js` resolves every mirror colour locally through
`resolveBuckets()` and emits literal SQL: 35 `UPDATE`s, one `DELETE`, 5
`INSERT`s, touching only `products.color`, `products.color_family` and the
`color_family_alt` rows on one category. **No CASE, no join to borrow another
row's family, no tie-break, no regex** — the file applies decisions, it does
not make any. Re-runnable.

Result: 106 mirrors with a family, 5 deliberately NULL (`Modern Iron` ×2,
`Natural` ×2, `Limestone` ×1), 21 alt rows.

### Still open after this work
- `Finish/Color of Product` writes EAV `finish` via `ATTR_MAP` **as well as**
  reaching `products.color`. Two homes for one fact — a Rule 8 question,
  deliberately not bundled into this change.
- The 57 Sunwashed Oak / Shagreen **vanity** products still carry the old
  `gray`/`green` families in the database. The code is fixed; only the mirrors
  were backfilled by SQL. They correct themselves on the next JM import.
