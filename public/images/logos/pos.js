$(document).ready(function() {
    var isCardPayment = 0;
    $("#credit-card-fee-container").css('display','none');
    
    // $(document).on("change",'#card_type_0',function(){
    //     alert($(this).val());
    // });
    $(document).on("change","#card_type_0",function(){
        ...
        if($(this).val()=="credit"){
            var feePayment = 0;
            if($(this).attr('id')=="card_type_0"){
                feePayment = $('#amount_0').val();
            }else if($(this).attr('id')=="card_type_1"){
                feePayment = $('#amount_1').val();
            }else if($(this).attr('id')=="card_type_2"){
                feePayment = $('#amount_2').val();
            }
            var total_amount = $('input#final_total_input').val();
             console.log("fee amount=>"+feePayment+"  total_amount=>"+total_amount);
            //Credit Card Fee
            var creditCardFee = 0
            total_amount = total_amount.replace(/,/g, '');
                console.log("equal amount",parseFloat(total_amount))
               creditCardFee = credit_card_fee(parseFloat(total_amount));
               total_amount = parseFloat(creditCardFee)+parseFloat(total_amount);
               updatePaymentRowAmount(total_amount)
            __write_number($('input#final_total_input'), parseFloat(total_amount));
            $('span.total_payable_span').text(__currency_trans_from_en(parseFloat(total_amount), true));
            $('span.total_paying').text(__currency_trans_from_en(parseFloat(total_amount), true));
            $('#total_paying_input').val(parseFloat(total_amount));
             if($(this).attr('id')=="card_type_0"){
                $('#amount_0').val(parseFloat(total_amount));
            }else if($(this).attr('id')=="card_type_1"){
                $('#amount_1').val(parseFloat(total_amount));
            }else if($(this).attr('id')=="card_type_2"){
                $('#amount_2').val(parseFloat(total_amount));
            }
            
            $('span#cc_fee').text(__currency_trans_from_en(creditCardFee.toFixed(2), false));
            $('#credit_card_fee_input').val(creditCardFee.toFixed(2));
            $("#credit-card-fee-container").css('display','block');
            isCardPayment = 1;
        }else{
            
                var price_total = $('#final_total_input').val();
                console.log(price_total,"=====price total");
                price_total = price_total.replace(/,/g, '')
                var fee = $('#credit_card_fee_input').val();
                var price_total = parseFloat(price_total)-parseFloat(fee);
                $('span#cc_fee').text(__currency_trans_from_en(0, false));
                $('#credit_card_fee_input').val(0);
                __write_number($('input#final_total_input'), parseFloat(price_total));
                $('span.total_payable_span').text(__currency_trans_from_en(parseFloat(price_total), true));
                $('span.total_paying').text(__currency_trans_from_en(parseFloat(price_total), true));
                $('#total_paying_input').val(parseFloat(price_total));
             if($(this).attr('id')=="card_type_0"){
                $('#amount_0').val(parseFloat(price_total));
            }else if($(this).attr('id')=="card_type_1"){
                $('#amount_1').val(parseFloat(price_total));
            }else if($(this).attr('id')=="card_type_2"){
                $('#amount_2').val(parseFloat(price_total));
            }
    ...
    $('#credit_card_fee_input').on('change',function(){
        let value = $('span#cc_fee').text();
        console.log(value,'====onchange');
        let total_amount = $('input#final_total_input').val();
    ...
    function updatePaymentRowAmount(total_amount){
        if($(this).attr('id')=="method_0"){
                 $('#amount_0').val(parseFloat(total_amount));
            }else if($(this).attr('id')=="method_1"){
                 $('#amount_1').val(parseFloat(total_amount));
            }else if($(this).attr('id')=="method_2"){
                 $('#amount_3').val(parseFloat(total_amount));
            }
    }
    
    $(document).on("change","#amount_0",function(){
        // alert($(this).val());
            var payingWithCard = $(this).val();
            var payment_method_dropdown = $('#payment_rows_div')
            .find('.payment_types_dropdown')
            .first();
        console.log(payment_method_dropdown.val(),"======payment_method");
        
        if(payment_method_dropdown.val() == 'card'){
                var calculation_type = 'percentage';
            // var calculation_amount = __read_number(10);
            var calculation_amount =2;
            var total_amount = payingWithCard;
            var creditCardFee = __calculate_amount(calculation_type, calculation_amount, total_amount);
            var price_total = $('#final_total_input').val();
            var updatePayable = parseFloat(price_total)-parseFloat(creditCardFee);
              $('#credit_card_fee_input').val(creditCardFee);
            __write_number($('input#final_total_input'), parseFloat(updatePayable));
            $('span.total_payable_span').text(__currency_trans_from_en(parseFloat(updatePayable), true));
        }
    });
    $("select#order_tax_modal option[data-location='" + $('select#select_location_id').val() + "']").prop("selected", true);
     var tax_obj = $('select#order_tax_modal');
        ...
        console.log(tax_rate,"============tax rate",$('span#order_tax'))
        $('span#order_tax').text(tax_rate)
    ...
    $('select#select_location_id').change(function() {
        // alert($(this).val());
        reset_pos_form();
$("select#order_tax_modal option[data-location='" + $(this).val() + "']").prop("selected", true);
        var default_price_group = $(this).find(':selected').data('default_price_group')
        console.log($('select#select_location_id').val(),'========val=====pg',default_price_group)
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        // Show VIP discount choice popup for tier 1 (vip_1) and tier 2 (vip_2) customers
        if ((data.rewards_type === 'vip_1' || data.rewards_type === 'vip_2') && parseFloat(data.discount_amount) > 0) {
            var tierName = data.rewards_type === 'vip_1' ? 'VIP Points Tier 1' : 'VIP Points Tier 2';
            var discountPct = parseFloat(data.discount_amount);
            $('#vip_discount_as_reward').val('0');
            $('#vip_tier_name').text(tierName);
            $('#vip_discount_pct_display').text(discountPct);
            $('#vipDiscountChoiceModal').data('discount_pct', discountPct).modal('show');
        } else {
            // Non-VIP customer: reset any previous VIP discount choice
            $('input#vip_discount_as_reward').val('0');
            __write_number($('input#discount_amount'), __read_number($('input#discount_amount').data('default') || 0));
    ...
    $('button#pos-finalize').click(function() {
        ...
        if ($('#types_of_service_id').length && !$('#types_of_service_id').val()) {
            toastr.warning('Please select a type of service before proceeding to payment');
            return false;
        }
    ...
    // VIP discount choice modal handlers
    $(document).on('click', '#vip_use_discount_btn', function() {
        var discountPct = $('#vipDiscountChoiceModal').data('discount_pct');
        $('input#discount_type').val('percentage');
        __write_number($('input#discount_amount'), discountPct);
        $('input#vip_discount_as_reward').val('0');
        $('input#vip_gross_total').val('0');
        pos_total_row();
        $('#vipDiscountChoiceModal').modal('hide');
    });

    $(document).on('click', '#vip_store_reward_btn', function() {
        var grossTotal = get_subtotal();
        $('input#vip_gross_total').val(grossTotal);
        __write_number($('input#discount_amount'), $('input#discount_amount').data('default'));
        $('input#discount_type').val($('input#discount_type').data('default'));
        $('input#vip_discount_as_reward').val('1');
        pos_total_row();
        $('#vipDiscountChoiceModal').modal('hide');
    });

    $(document).on('click', '#vip_modal_close_btn', function() {
        __write_number($('input#discount_amount'), $('input#discount_amount').data('default'));
        $('input#discount_type').val($('input#discount_type').data('default'));
        $('input#vip_discount_as_reward').val('0');
        $('input#vip_gross_total').val('0');
...
function credit_card_fee(price_total) {
    var tax_rate_id = $('#tax_rate_id').val();
    var calculation_type = 'percentage';
    // var calculation_amount = __read_number(10);
        var calculation_amount =2;
...
function reset_pos_form(){
	...
	$('span.total_quantity, span.price_total, span#total_discount, span#total_payable, span#shipping_charges_amount').text(0);
	...
	// Uncheck the send review link checkbox
	if ($('#send_review_link_checkbox').length) {
    $('#send_review_link_checkbox').iCheck('uncheck');
}
...
$(document).on('change', 'select#customer_id', function(){
    ...
    if ($(this).val() == default_customer_id) {
        ...
        // Reset VIP discount state for walk-in customer
        $('input#vip_discount_as_reward').val('0');
    } else {
        if ($('#rp_redeemed_modal').length) {
