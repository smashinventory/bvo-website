...
const comboEstimator = require('../services/comboDemandEstimator');
...
async function dashboard(req, res) {
  try {
    ...
    /* ── Estimated Combo Demand ───────────────────────────────────────
       The estimator MOVED to src/services/comboDemandEstimator.js on
       2026-09-12. It is unchanged — the extracted SQL is byte-identical to
       what stood here, proven by test/fixtures/combo_estimate_before.sql.

       It moved because products.demand_score has to carry the same numbers
       for the storefront sort (definition §8), and the nightly rollup could
       not reach a query embedded in a controller. They cannot share a stored
       result — §3a requires the warranty overage to recompute per window and
       this report is window-parameterised — so they share the SQL.

       Rules 1 to 4, the rc_collections band, the warranty adjustment and the
       ROUND(...,2) all live there now. Read that file, not this comment. */
    const comboLeaderboard = hasDims ? await safeQuery(
      comboEstimator.comboEstimateSql(40),
      comboEstimator.comboEstimateParams(cutoffStr)) : [];

    /* Rule 3 — Bellamy (D300), the one captive combo. Also in the module. */
    const captiveCombos = hasDims ? await safeQuery(
      comboEstimator.CAPTIVE_SQL,
      comboEstimator.captiveParams(cutoffStr)) : [];
